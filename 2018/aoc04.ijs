...
parseRecords=: [: ([: }.&.> ']'&cut);._2  '- : ' charsub ('Guard ';'') rplc~ ]
getTimeStamp=: _999 ". [: > {."1
sortRecords=: (/: getTimeStamp)@parseRecords
getMins=: {:"1@getTimeStamp

grpShifts=: ('#'={.)&>@:{:"1 <@}.;.1 getMins
guardShifts=: (('#'={.)&>@:{:"1 # ((_99 ". ' ' taketo }.)&>)@:{:"1)
mskSleep=: 1:`(({. + [: i. -~/)@])`(60 $ 0:)}~
mskShift=: +/@(_2 mskSleep\ ;)

getGuardSleepMins=: guardShifts mskShift/. grpShifts

getGuardMin_s1=: verb define
  recs=. sortRecords y
  idx=. (i. >./)@:(+/"1) getGuardSleepMins recs
  min=. (i. >./) idx { getGuardSleepMins recs
  id=. idx { ~.guardShifts recs
  min * id
)
...
test_outputs=: 240
assert test_outputs = getGuardMin_s1 test_inputs

input=: freads '2018/aoc04_input.txt'

'Day 4, Part 1 answer: %d' printf getGuardMin_s1 input
...
getGuardMin_s2=: verb define
  recs=. sortRecords y
  idx=. ,(4 $.$.) (= >./@,) getGuardSleepMins recs
  min=. {: idx
  id=. ({.idx) { ~.guardShifts recs
  min * id
)
...
'Day 4, Part 2 answer: %d' printf getGuardMin_s2 input
