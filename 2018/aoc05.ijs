Note 'Day 5: Alchemical Reduction'
...
diff32=: 0 ,~ 32 = 2 |@-/\ a.&i.   NB. the ascii neighbour to right differs by 32
firstones=: > 0: , }:
mask2nd=: + _1&|.
reactPolymer=: (#~ -.@mask2nd@firstones@diff32)^:_

test_inputs=: 'dabAcCaCBAcCcaDA'
test_outputs=: 'dabCBAcaDA'
test_outputs=: 10
assert test_outputs = #@reactPolymer test_inputs

input=: }: freads '2018/aoc05_input.txt'

'Day 5, Part 1 answer: %d' printf #@reactPolymer input
...
removeReactPolymer=: (a. {~ 65 97 +/~ i.26) <@reactPolymer@-."1~ ]

test_inputs=: test_inputs  NB. same as part 1
test_outputs=: 4
assert test_outputs = <./ #&> removeReactPolymer test_inputs

'Day 5, Part 2 answer: %d' printf <./ #&> removeReactPolymer input

Note 'Alternative faster solution by jonah'
p1 =. [: # ,`(}.@])@.(32 = |@(- {.)&(a.&i.))/
